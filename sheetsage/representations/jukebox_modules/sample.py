...
from jukebox_modules.utils.sample_utils import get_starts, split_batch
...
def sample_single_window(zs, labels, sampling_kwargs, level, prior, start, hps):
    ...
    z_samples = []
    for z_i, z_conds_i, y_i in zip(z_list, z_conds_list, y_list, strict=False):
        z_samples_i = prior.sample(
            n_samples=z_i.shape[0], z=z_i, z_conds=z_conds_i, y=y_i, **sampling_kwargs
        )
...
def _sample(zs, labels, sampling_kwargs, priors, sample_levels, hps, device=None):
    ...
    for level in reversed(sample_levels):
        ...
        assert hps.sample_length % prior.raw_to_tokens == 0, (
        ...
        zs = sample_level(
            zs, labels[level], sampling_kwargs[level], level, prior, total_length, hop_length, hps
        )
