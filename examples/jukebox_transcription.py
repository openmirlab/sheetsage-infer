...
def transcribe_with_hints(
    audio_path, start_time=None, end_time=None, bpm=None, output_dir="./output"
):
    """
...
Note: Jukebox features require GPU with >=12GB VRAM and are slower
    ...
    parser.add_argument(
        "audio_path", type=str, help="Path to the audio file (supports .mp3, .wav, .flac, etc.)"
    )
    parser.add_argument(
        "-o",
        "--output_dir",
    ...
    parser.add_argument(
        "-e", "--end", type=float, default=None, help="Approximate end time in seconds (optional)"
    )
    parser.add_argument(
        "-b",
        "--bpm",
