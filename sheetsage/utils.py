...
def _check_command(cmd, alternative=None):
    """Check if a command exists in PATH."""
    if shutil.which(cmd):
        return cmd
    if alternative and shutil.which(alternative):
        return alternative
    alt_msg = f"Alternative '{alternative}' also not found. " if alternative else ""
    raise FileNotFoundError(
        f"Command '{cmd}' not found in PATH. {alt_msg}"
        f"Please install the required tool (e.g., pip install {cmd} or system package manager)."
    )
...
def encode_audio(path, sr, audio, bitexact=False, timeout=60.0):
    ...
    """
    _check_command("ffmpeg")  # Check before use
...
def get_approximate_audio_length(path, timeout=10):
    ...
    _check_command("ffprobe")  # Check before use
    status, stdout, stderr = run_cmd_sync(
