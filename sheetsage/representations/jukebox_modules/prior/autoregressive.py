...
from jukebox_modules.transformer.ops import filter_logits
from jukebox_modules.transformer.transformer import Transformer
from jukebox_modules.utils.logger import get_range
...
class ConditionalAutoregressive2D(nn.Module):
    ...
    def forward(
        ...
        x = (
            self.x_emb_dropout(x) + self.pos_emb_dropout(self.pos_emb()) + x_cond
        )  # Pos emb and dropout
        ...
        if get_sep_loss:
            ...
            prime_loss = F.cross_entropy(x_prime, x_t[:, : self.prime_len].reshape(-1)) / np.log(
                2.0
            )
    ...
    def get_emb(self, sample_t, n_samples, x, x_cond, y_cond):
        ...
        if sample_t == 0:
            # Fill in start token - get device from model parameters
            device = next(self.parameters()).device
            x = t.empty(n_samples, 1, self.width, device=device)
            if self.y_cond:
                x[:, 0] = y_cond.view(N, self.width)
            else:
                x[:, 0] = self.start_token
    ...
    def sample(
        ...
        if self.x_cond:
            ...
            assert x_cond is None
            device = next(self.parameters()).device
            x_cond = t.zeros((N, 1, self.width), dtype=t.float, device=device)
    ...
    def check_sample(self, chunk_size):
        ...
        with t.no_grad():
            device = next(self.parameters()).device
            y_cond = t.randn(bs, 1, d, device=device) if self.y_cond else None
            x_cond = t.randn(bs, l, d, device=device) if self.x_cond else None
            encoder_kv = t.randn(bs, enc_l, d, device=device)
