...
class FactoredAttention(nn.Module):
    def __init__(
        ...
        self.n_in = n_in
        self.n_ctx = (
            n_ctx  # NOTE: n_ctx could be different within operations. This is complete n_ctx
        )
    ...
    def dense_attn(self, query, key, value, sample):
        ...
        if self.checkpoint_attn == 1 and not sample:
            a = checkpoint(
                lambda q, k, v, s=sample: self._attn(q, k, v, s), (query, key, value), (), True
            )
        else:
    ...
    def summary_attn(self, q, k, v, sample):
        ...
        if sample:
            k = t.nn.functional.pad(
                k[:, block_ctx - 1 : blocks * block_ctx - 1 : block_ctx, :], (0, 0, 1, 0)
            )
            v = t.nn.functional.pad(
                v[:, block_ctx - 1 : blocks * block_ctx - 1 : block_ctx, :], (0, 0, 1, 0)
            )
            return self.dense_attn(q, k, v, sample).view(bs, 1, d)
        else:
            k = t.nn.functional.pad(
    ...
    def summary_spread_attn(self, q, k, v, sample):
        ...
        if sample:
            ...
            k = (
                t.nn.functional.pad(
                    k.view(bs, blocks, l // blocks, d)[:, :-1, -spread:, :], (0, 0, 0, 0, 1, 0)
                )
                .contiguous()
            ...
            v = (
                t.nn.functional.pad(
                    v.view(bs, blocks, l // blocks, d)[:, :-1, -spread:, :], (0, 0, 0, 0, 1, 0)
                )
                .contiguous()
                .view(bs, blocks * spread, d)
            )  # bs, blocks * spread, d
    ...
    def decode_attn(self, q, k, v, sample):
        assert k.shape[1] == v.shape[1] == self.encoder_dims, (
    ...
    def prime_qkv(self, x, encoder_kv=None, sample=False):
        ...
        if sample:
            ...
            assert key.shape[1] == value.shape[1] == self.n_ctx, (
    ...
    def decode_qkv(self, x, encoder_kv=None, sample=False):
        ...
        if sample:
            if self.sample_t == 0:
                self.cache["key"], self.cache["value"] = self.c_enc_kv(encoder_kv.type_as(x)).chunk(
                    2, dim=2
                )
            key, value = self.cache["key"], self.cache["value"]
            self.sample_t += curr_ctx
        ...
        assert key.shape[1] == value.shape[1] == self.encoder_dims, (
    ...
    def forward(self, x, encoder_kv=None, sample=False):
        ...
        if self.checkpoint_attn == 2 and not sample:
            a = checkpoint(
                lambda q, k, v, s=sample: self.attn(q, k, v, s), (query, key, value), (), True
            )
        else:
    ...
    def check(self):
        ...
        x = t.randn(bs, l, d, device=device)
        ...
        exp_pos_grad = {
            ...
            5: (
                (t.arange(pos) % (l // blocks) >= (l // blocks - spread))
                & (t.arange(pos) < block_pos)
            )
            .nonzero()
            .view(-1),
        }[self.attn_func]
    ...
    def check_cache(self, n_samples, sample_t, fp16):
        ...
        if sample_t == 0:
            ...
            assert self.cache["key"].dtype == dtype, (
                f"Expected {dtype}, got {self.cache['key'].dtype}"
            )
            assert self.cache["value"].dtype == dtype, (
