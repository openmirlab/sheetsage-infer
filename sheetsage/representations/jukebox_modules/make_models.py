...
import numpy as np
import torch as t
from jukebox_modules.hparams import REMOTE_PREFIX, setup_hparams
from jukebox_modules.utils.dist_utils import print_all
from jukebox_modules.utils.remote_utils import check_file_exists, download
...
def load_checkpoint(path, auto_download=True):
    ...
    if file_size < 1024:  # Less than 1KB is definitely corrupted
        if dist.get_rank() == 0:
            print(
                f"⚠ Checkpoint file appears corrupted (size: {file_size} bytes). Removing and re-downloading..."
            )
        os.remove(restore)
        ...
        if path.startswith(REMOTE_PREFIX):
            ...
            local_path = os.path.join(
                os.path.expanduser("~/.cache"), remote_path[len(REMOTE_PREFIX) :]
            )
            if dist.get_rank() == 0:
                download(remote_path, local_path, show_progress=True)
            dist.barrier()
            ...
            raise FileNotFoundError(
                f"Corrupted checkpoint removed: {restore}. Please re-download manually."
            )
    ...
    try:
        ...
        if dist.get_rank() == 0:
            ...
            print("  Removing corrupted file and attempting re-download...")
        os.remove(restore)
        ...
        if path.startswith(REMOTE_PREFIX):
            ...
            try:
                ...
                raise RuntimeError(
                    f"Checkpoint still corrupted after re-download: {e2}. "
                    f"Please manually download: {remote_path}"
                )
        else:
            raise RuntimeError(
                f"Checkpoint file corrupted and cannot be re-downloaded (local path): {restore}. "
                f"Error: {e}"
            )
...
def make_prior(hps, vqvae, device="cuda", auto_download=True):
    ...
    def rescale(z_shape):
        return (z_shape[0] * hps.n_ctx // vqvae.z_shapes[hps.level][0],)
...
def make_model(model, device, hps, levels=None, auto_download=True):
    ...
    priors = [
        make_prior(setup_hparams(priors[level], {}), vqvae, device, auto_download=auto_download)
        for level in levels
    ]
    return vqvae, priors
