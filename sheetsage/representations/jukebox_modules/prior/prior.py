...
from jukebox_modules.data.labels import EmptyLabeller, Labeller
from jukebox_modules.prior.autoregressive import ConditionalAutoregressive2D
from jukebox_modules.prior.conditioners import Conditioner, LabelConditioner
from jukebox_modules.transformer.ops import LayerNorm
from jukebox_modules.utils.dist_utils import print_once
from jukebox_modules.utils.torch_utils import assert_shape
...
class SimplePrior(nn.Module):
    def __init__(
        ...
        if self.y_cond:
            self.n_time = self.z_shape[
                0
            ]  # Assuming STFT=TF order and raw=T1 order, so T is first dim
            self.y_emb = LabelConditioner(
                n_time=self.n_time, include_time_signal=not self.x_cond, **y_cond_kwargs
            )
        ...
        if single_enc_dec:
            ...
            if self.n_tokens != 0 and self.use_tokens:
                from jukebox_modules.transformer.ops import Conv1D
    ...
    def prior_preprocess(self, xs, conds):
        ...
        for i in range(len(xs)):
            x, _shape, dims = xs[i], self.prior_shapes[i], self.prior_dims[i]
        ...
        for i in range(len(conds)):
            cond, _shape, dims = conds[i], self.prior_shapes[i], self.prior_dims[i]
            if cond is not None:
                assert_shape(cond, (N, dims, self.prior_width))
            else:
                # Get device from model parameters instead of hardcoding CUDA
                device = next(self.parameters()).device
                conds[i] = t.zeros((N, dims, self.prior_width), dtype=t.float, device=device)
    ...
    def x_emb(self, z_conds):
        ...
        assert len(z_conds) == len(self.conditioner_blocks) == self.cond_level - self.level, (
            f"Expected {len(z_conds)} == {len(self.conditioner_blocks)} == {self.cond_level} - {self.level}"
        )
        x_cond = None
        for z_cond, conditioner_block in reversed(list(zip(z_conds, self.conditioner_blocks, strict=False))):
    ...
    def decode(self, zs, start_level=None, end_level=None, bs_chunks=1):
        ...
        with t.no_grad():
            x_out = self.decoder(
                zs, start_level=start_level, end_level=end_level, bs_chunks=bs_chunks
            )
        return x_out

    def get_cond(self, z_conds, y):
        if y is not None:
            assert y.shape[1] == 4 + self.y_emb.max_bow_genre_size + self.n_tokens, (
    ...
    def get_encoder_kv(self, prime, fp16=False, sample=False):
        if self.n_tokens != 0 and self.use_tokens:
            # Get device from parent model (self) instead of hardcoding CUDA
            device = next(self.parameters()).device
            if sample:
                self.prime_prior.to(device)
            N = prime.shape[0]
            ...
            if sample:
                # Move to CPU to save memory only if not already on CPU
                if device.type != "cpu":
    ...
    def get_prime_loss(self, encoder_kv, prime_t):
        if self.use_tokens:
            ...
            # Get device from model parameters instead of hardcoding CUDA
            device = next(self.parameters()).device
            prime_loss = t.tensor(0.0, device=device)
        return prime_loss

    def z_forward(self, z, z_conds=None, y=None, fp16=False, get_preds=False, get_attn_weights=False):
        """
        Arguments:
        ...
        if self.single_enc_dec:
            ...
            gen_loss, preds = self.prior(
                z, x_cond, y_cond, encoder_kv, fp16=fp16, get_preds=get_preds
            )
        loss = (
            self.prime_loss_fraction * prime_loss * self.prime_loss_dims / self.total_loss_dims
