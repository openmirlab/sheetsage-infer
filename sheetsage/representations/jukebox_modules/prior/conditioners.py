...
from jukebox_modules.vqvae.encdec import DecoderConvBock
...
class RangeEmbedding(nn.Module):
    ...
    def forward(self, pos_start, pos_end=None):
        ...
        assert (self.pos_min <= pos_start).all() and (pos_start < self.pos_max).all(), (
        ...
        if pos_end is not None:
            ...
            assert (self.pos_min <= pos_end).all() and (pos_end <= self.pos_max).all(), (
        ...
        if n_time != 1:
            ...
            interpolation = (
                t.arange(0, n_time, dtype=t.float, device=device).view(1, n_time) / n_time
            )
        ...
        bins = (
            (self.bins * normalised_position).floor().long().detach()
        )  # [0,1) -> [0,1..,bins) -> [0,1...,bins-1]
...
class LabelConditioner(nn.Module):
    def __init__(
        ...
        if self.include_time_signal:
            ...
            assert len(t_ranges) == 3, (
                f"Expecting (total, absolute, relative) ranges, got {t_ranges}"
            )
            total_length_range, absolute_pos_range, relative_pos_range = t_ranges
            self.total_length_emb = RangeEmbedding(
                1, t_bins, total_length_range, out_width, init_scale
            )
            self.absolute_pos_emb = RangeEmbedding(
                n_time, t_bins, absolute_pos_range, out_width, init_scale
            )
    ...
    def forward(self, y):
        assert len(y.shape) == 2, f"Expected shape with 2 dims, got {y.shape}"
        assert y.shape[-1] == 4 + self.max_bow_genre_size, (
