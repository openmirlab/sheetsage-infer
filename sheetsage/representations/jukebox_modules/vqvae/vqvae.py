...
from jukebox_modules.utils.audio_utils import (
    audio_postprocess,
    multispectral_loss,
    spectral_convergence,
    spectral_loss,
)
from jukebox_modules.utils.logger import average_metrics
from jukebox_modules.vqvae.bottleneck import Bottleneck, NoBottleneck
from jukebox_modules.vqvae.encdec import Decoder, Encoder, assert_shape
...
def calculate_strides(strides, downs):
    return [stride**down for stride, down in zip(strides, downs, strict=False)]
...
def _loss_fn(loss_fn, x_target, x_pred, hps):
    if loss_fn == "l1":
        ...
        raise AssertionError(f"Unknown loss_fn {loss_fn}")
...
class VQVAE(nn.Module):
    ...
    def encode(self, x, start_level=0, end_level=None, bs_chunks=1):
        ...
        for x_i in x_chunks:
            ...
            zs_list.append(zs_i)
        zs = [t.cat(zs_level_list, dim=0) for zs_level_list in zip(*zs_list, strict=False)]
        return zs
    ...
    def sample(self, n_samples):
        ...
        zs = [
            t.randint(0, self.l_bins, size=(n_samples, *z_shape), device=device)
